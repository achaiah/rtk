...
mod tests {
    ...
    fn test_extract_identifier_with_repo_flag_after() {
        // gh issue view 185 -R achaiah/rtk
        let args: Vec<String> = vec!["185".into(), "-R".into(), "achaiah/rtk".into()];
        let (id, extra) = extract_identifier_and_extra_args(&args).unwrap();
        assert_eq!(id, "185");
        assert_eq!(extra, vec!["-R", "achaiah/rtk"]);
    }

    #[test]
    fn test_extract_identifier_with_repo_flag_before() {
        // gh issue view -R achaiah/rtk 185
        let args: Vec<String> = vec!["-R".into(), "achaiah/rtk".into(), "185".into()];
        let (id, extra) = extract_identifier_and_extra_args(&args).unwrap();
        assert_eq!(id, "185");
        assert_eq!(extra, vec!["-R", "achaiah/rtk"]);
    }
    ...
    fn test_extract_identifier_only_flags() {
        ...
        let args: Vec<String> = vec!["-R".into(), "achaiah/rtk".into()];
        assert!(extract_identifier_and_extra_args(&args).is_none());
    }
...
[![CI](https://img.shields.io/github/actions/workflow/status/achaiah/rtk/ci.yml)](https://github.com/achaiah/rtk/actions)
[![Coverage](https://img.shields.io/codecov/c/github/achaiah/rtk)](https://codecov.io/gh/achaiah/rtk)
